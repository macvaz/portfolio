...
def test_download_sp500_history_downloads_from_yfinance(monkeypatch):
    index = pd.to_datetime(["2000-01-03", "2000-01-04"])
    fake_history = pd.DataFrame(
        {"Close": [1455.22, 1394.46]},
        index=index,
    )
    ...
    monkeypatch.setattr("portfolio.datasources.sp500.yf.download", fake_download)
...
def test_load_backtest_sp500_csv_filters_date_range(tmp_path):
    ...
    from portfolio.datasources.sp500 import load_backtest_sp500_csv
